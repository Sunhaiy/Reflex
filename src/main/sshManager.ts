...
import { SSHConnection, SystemStats, FileEntry } from '../shared/types.js';
...
export class SSHManager {
  ...
  private prevCpu: { user: number, nice: number, system: number, idle: number, iowait: number, irq: number, softirq: number, steal: number } | null = null;
  ...
  async getSystemInfo(id: string): Promise<any> {
    ...
    return new Promise((resolve) => {
        ...
        const info: any = { os: 'N/A', cpu: {}, network: {} };
  ...
  async deleteFile(id: string, path: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        conn.sftp((err, sftp) => {
            ...
            const deleteRecursive = (p: string): Promise<void> => {
  ...
  async createDirectory(id: string, path: string): Promise<void> {
    const conn = this.connections.get(id);
    if (!conn) throw new Error('Not connected');
