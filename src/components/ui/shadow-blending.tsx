...
//
// Kept as a verbatim drop-in so it can be re-pulled from 21st.dev; the only local
// change is dropping the "use client" directive, which this Vite renderer has no
// use for and which the bundler warns about.
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Arrow consts rather than hoisted `function`s: a hoisted declaration counts as
    // created before the `canvas`/`gl` null guards above, so TypeScript drops the
    // narrowing inside it. Both are only ever called after both are defined.
    const requestRender = () => {
