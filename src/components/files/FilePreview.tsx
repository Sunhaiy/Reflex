...
export function FilePreview({ file, onClose, onEdit, onDownload }: Props) {
    const { t } = useTranslation();
    const [scale, setScale] = useState(1);
    const [rotation, setRotation] = useState(0);
    const [imageLoading, setImageLoading] = useState(file.kind === 'image');
    ...
    return (
        <div className="fixed inset-0 z-[65] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm" onMouseDown={onClose}>
            <section
                className="flex h-[min(760px,calc(100vh-32px))] w-[min(980px,calc(100vw-32px))] flex-col overflow-hidden rounded-2xl border border-border/70 bg-background shadow-2xl"
            ...
            >
                <header className="flex min-h-14 shrink-0 items-center justify-between gap-3 px-4">
                    ...
                    <div className="flex shrink-0 items-center gap-1 rounded-xl bg-foreground/[0.035] p-1">
                        {file.kind === 'image' && (
                            <>
                                <button className={actionClass} onClick={() => setScale((value) => Math.max(0.2, value - 0.2))} title={t('fileBrowser.zoomOut')}>
                                ...
                                <button className={actionClass} onClick={() => setScale((value) => Math.min(4, value + 0.2))} title={t('fileBrowser.zoomIn')}>
                                    <HugeiconsIcon icon={ZoomInAreaIcon} className="h-3.5 w-3.5" />
                                </button>
                                <button className={actionClass} onClick={() => setRotation((value) => (value + 90) % 360)} title={t('fileBrowser.rotate')}>
                                    <HugeiconsIcon icon={RotateRight01Icon} className="h-3.5 w-3.5" />
                                </button>
                            </>
                        )}
                        <button className={actionClass} onClick={onDownload}>
                        ...
                        <button className={actionClass} onClick={onClose} title={t('fileBrowser.closePreview')}>
                        ...
                        {file.kind === 'image' ? (
                            <div className="flex min-h-full min-w-full items-center justify-center p-6">
                                {imageLoading && !imageError && (
                                    <div className="absolute rounded-full bg-background/85 px-3 py-1.5 text-xs text-muted-foreground shadow-sm">
                                ...
                                {imageError ? (
                                    ...
                                    <img
                                        ...
                                        alt={file.name}
                                        draggable={false}
                                        className="max-w-none rounded-lg object-contain shadow-xl transition-[transform,opacity] duration-200"
                                        style={{
